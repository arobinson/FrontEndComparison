...
export async function exportToJSON(data: FullTestResults, outputPath: string): Promise<void> {
  const json = JSON.stringify(data, null, 2);
  await writeFile(outputPath, json, 'utf-8');
  console.log(`\nFull results exported to: ${outputPath}`);
}

export async function exportAggregatedToJSON(aggregated: AggregatedResults[], outputPath: string): Promise<void> {
